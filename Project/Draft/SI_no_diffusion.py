...
sol = sp.integrate.solve_ivp(
    rhs, t[[0, -1]], [S0, I0], t_eval=t, args=(beta, gamma))
h = sol.t[1] - sol.t[0]

Phi = np.column_stack(phi(sol.y[0], sol.y[1]))
Phi = Phi[1:-1]
...
for alpha in (alpha_sweep):
    ...
    feature_names = ['S', 'I', 'SI']
    theta_S = S_fit.coef_
    theta_I = I_fit.coef_
    theta_summary = np.column_stack([feature_names, theta_S, theta_I])
    ...
    for threshold in (threshold_sweep):
        ...
        theta_columns = ("Basis Function", "Alpha", "Threshold",
                         "Theta S", "Theta I")
